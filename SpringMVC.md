...
<servlet>
    ...
    <!-- 
 		DispatcherServlet作为框架的核心组件，在启动过程中有大量的初始化操作要做
...
>
> /*则能够匹配所有请求，例如在使用过滤器时，若需要对所有请求进行过滤，就需要使用/星的写法
...
// @RequestMapping注解的value属性可以通过请求地址匹配请求，/表示的当前工程的上下文路径（工程路径）
...
可以在控制器方法的形参位置设置一个实体类类型的形参，此时若浏览器传输的请求参数的参数名和实体类中的属性名一致，那么请求参数就会为此属性赋值（得有对应的set方法）
...
解决获取请求参数（**请求方式为post**）的乱码问题，可以使用SpringMVC提供的编码过滤器CharacterEncodingFilter，但是必须在web.xml中进行注册
...
> 以下1-5种向作用域存储数据的方式，最后都封装为modelAndView类型并返回给前端控制器
...
![image-20220213203034199](https://gitee.com/steadyHeart/drawingBed/raw/master/photos/image-20220213203034199.png)

iii. 如果有配置，则访问目标资源（一般为静态资源，如：JS,CSS,HTML），找不到客户端也会展示404错误

![image-20220213202931120](https://gitee.com/steadyHeart/drawingBed/raw/master/photos/image-20220213202931120.png)
